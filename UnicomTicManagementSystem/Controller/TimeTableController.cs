    ...
    public class TimeTableController
    {
        public async Task<bool> AddTimetableAsync(Timetable timetable)
        {
            try
            {
                ...
                {
                    // Check for clashes: same room or lecturer at same time and day
                    string clashQuery = @"
                        SELECT COUNT(*) FROM Timetables 
                        WHERE Day = @Day AND TimeSlot = @TimeSlot
                        AND (RoomID = @RoomID OR UserID = @UserID)";

                    using (var clashCmd = new SQLiteCommand(clashQuery, conn))
                    {
                        clashCmd.Parameters.AddWithValue("@Day", timetable.Day);
                        clashCmd.Parameters.AddWithValue("@TimeSlot", timetable.TimeSlot);
                        clashCmd.Parameters.AddWithValue("@RoomID", timetable.RoomID);
                        clashCmd.Parameters.AddWithValue("@UserID", timetable.UserID);

                        long count = (long)await clashCmd.ExecuteScalarAsync();
                        if (count > 0)
                        {
                            // Clash found
                            return false;
        ...
        public async Task<bool> UpdateTimetableAsync(Timetable timetable)
        {
            try
            {
                using (var conn = DatabaseManager.GetConnection())
                {
                    // Check for clashes except current record
                    string clashQuery = @"
                        SELECT COUNT(*) FROM Timetables 
                        WHERE Day = @Day AND TimeSlot = @TimeSlot
                        AND (RoomID = @RoomID OR UserID = @UserID)
                    ...
                    using (var clashCmd = new SQLiteCommand(clashQuery, conn))
                    {
                        ...
                        clashCmd.Parameters.AddWithValue("@RoomID", timetable.RoomID);
                        clashCmd.Parameters.AddWithValue("@UserID", timetable.UserID);
                        clashCmd.Parameters.AddWithValue("@TimetableID", timetable.TimetableID);
